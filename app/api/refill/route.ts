...
async function sendEmailViaResend(data: any) {
  ...
  const subject = `New refill request — ${patientName || 'Unknown'}`
  ...
  await resend.emails.send({
    ...
    to: pharmacyEmail,
    replyTo: email || fromEmail, // Always set Reply-To to patient's email if available
    subject,
    text: plainBody,
    html: htmlBody,
  })
